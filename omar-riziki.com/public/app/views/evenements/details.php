...
<form action="<?php  echo URLROOT. "/participer" ?>" method="post">
    <h1>Détails de <?= $data->libelle ?></h1>
    <p>ID : <?= $data->idev ?></p>

    <p>Prix : <?= $data->prix?></p>
    <p>Details : <?=$data->details ?></p>
    <p>Description  :<?=$data->details ?> </p>
    <p>cet evenement aura lieu le : <strong><?=$data->date ?></strong> à <strong><?= $data->adresse?></strong></p>
   ...
   <input type='submit' name='register' class='form-control bg-theme' value='Participer<?=$data->idev?>'>
   <?php else : ?>
